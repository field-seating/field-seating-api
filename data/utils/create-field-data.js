...
// csv read
async function getSpacesData(folder) {
  const files = await fs.promises.readdir(folder);
  let data = [];
  for (let file of files) {
    const fileContent = await fs.promises.readFile(`${folder}/${file}`); // data of space
...
async function createData(fieldData, folder) {
  logger.info(`start to create ${fieldData.fields.name}field data`);
  ...
  // create space
  const spacesData = await getSpacesData(folder);
  await PromisePool.withConcurrency(POOL_SIZE)
    .for(spacesData)
    .process(async (space) => {
      // if fieldId never get
